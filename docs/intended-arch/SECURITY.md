...
> **Status:** Active · **Updated:** 2026-03-27 · **Companion to:** [ARCHITECTURE.md](ARCHITECTURE.md)
...
|---|---|---|
| Runtime | Node.js 24 (single-threaded event loop) | npm supply chain, no memory safety |
| Skill system | Markdown files + shell scripts | No sandboxing, full machine access |
| Skill distribution | npm registry (ClawHub) | Unvetted submissions, Cisco found exfiltration |
...
|---|---|---|
| **Language safety** | JavaScript (Node.js) | Rust (memory safe, compile-time guarantees) |
...
| **Supply chain** | npm | Rust crates with cargo-deny + cargo-audit |
...
**Springtale covers every OpenClaw feature in Phase 2 with equal or superior
security, privacy, and architectural quality. There are zero gaps.**
...
### 1.3 Migration Path for OpenClaw Users
...
| Crate | Version | Advisory Status | Risk | Notes |
|---|---|---|---|---|
| `wasmtime` | **≥41.0.4** | CVE-2026-24116: buffer overflow on x86-64 AVX (fcopysign), fixed 41.0.1. CVE-2026-27572: wasi-http header DoS (32K header panic crashes host), fixed **41.0.4**. CVE-2025-62711: component-model trampoline crash, fixed 38.0.3. | **High (if unpinned)** | We pin ≥41.0.4 to cover all known CVEs. Bytecode Alliance maintains. 24/7 OSS-Fuzz. Formal verification of Cranelift ongoing. `Config::signals_based_traps(true)` default mitigates CVE-2026-24116 impact. `Config::max_wasm_http_fields_size` added in patch for CVE-2026-27572. |
| `wasmtime-wasi` | **≥41.0.4** | Same as wasmtime. | **High (if unpinned)** | Same pin requirement. |
