...
- **Channel list:** Last-writer-wins per channel_id on archived flag. ChannelCreated sets archived=false, ChannelArchived sets archived=true. Highest Lamport wins, tiebreak: lowest pseudonym key.
- **Role definitions:** Last-writer-wins per role_id (highest Lamport, tiebreak: lowest pseudonym key).
- **Role assignments:** Last-writer-wins per member_pseudonym (highest Lamport, tiebreak: lowest pseudonym key).
- **Ban list:** Last-writer-wins per pseudonym on banned/unbanned flag. Collect all BanEntry and UnbanEntry for each pseudonym; highest Lamport wins, tiebreak: lowest pseudonym key.
- **Community metadata:** Last-writer-wins on single entry (highest Lamport).
- **MEK generation:** Highest generation number wins. Tiebreak: lowest rotator pseudonym key (deterministic).
