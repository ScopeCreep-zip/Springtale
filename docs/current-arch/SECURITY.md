...
| **V10: Malicious Code** | Connector sandboxing (Wasmtime fuel + memory limits). Ed25519 manifest signatures are required: an unsigned manifest, or one signed by an author not in the trusted-author list, is rejected at install and on every load. Developers sign their own connectors with `springtale author add --self` then `springtale connector sign <manifest>`. `cargo deny` + `cargo audit` for supply chain. No eval/exec in Rust. TypeScript only in WASM sandbox. | SCA pipeline, manifest signature verification tests |
...
| — | WebSocket Hijack / ClawJacked (Oasis Security, CVE-2026-25253) | OpenClaw's localhost WebSocket assumed local = trusted. Springtale has no WebSocket gateway. Management API is HTTP REST with issued bearer tokens, `tower-http` rate limiting, and `127.0.0.1` default binding. The ClawJacked attack vector does not exist. |
...
| **ASI04** | Supply Chain Vulnerabilities | Ed25519 signed manifests. WASM sandbox. `cargo deny` + `cargo audit`. Connector WASM scanned by `trivy` + `wasm-validate`. No runtime loading of unsigned components; unsigned or unknown-author manifests are rejected. Registry rejects unsigned uploads. |
...
| **ASI09** | Human-Agent Trust Exploitation | Consent separation: high-stakes actions (send email, delete files, financial transactions, create new integrations) go through the approval gate, which asks a human — surfaced in chat and in the dashboard (`GET /approvals`, `POST /approvals/{id}`) — rather than default-denying. The agent cannot approve its own requests. Confidence scoring: AI adapter responses include a confidence field; low-confidence actions require human review. Action impact classification: each connector action carries MCP `readOnlyHint` / `destructiveHint` semantics; an action whose destructiveness is unknown counts as destructive. Every destructive action goes through the approval gate. |
