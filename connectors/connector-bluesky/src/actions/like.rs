...
pub fn declaration() -> ActionDecl {
    ActionDecl {
        name: "like".to_owned(),
        description: "Like a Bluesky post.".to_owned(),
        input_schema: Some(serde_json::json!({
            "type": "object",
            "properties": {
                "subject_uri": { "type": "string", "description": "AT URI of the post to like." },
                "subject_cid": { "type": "string", "description": "CID of the post to like." }
            },
...
pub async fn execute(
    ...
    Ok(ActionResult {
        ...
        message: format!("liked {subject_uri}"),
    })
}
...
mod tests {
    ...
    async fn test_execute_extracts_uri_from_response() {
        let mock = MockBlueskyClient {
            response: serde_json::json!({
                "uri": "at://did:plc:abc123/app.bsky.feed.like/3k2la",
                "cid": "bafylike"
            }),
        };
