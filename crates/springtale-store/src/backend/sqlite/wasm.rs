...
impl SqliteBackend {
    ...
    pub(super) async fn list_wasm_binaries_impl(
        &self,
    ) -> Result<Vec<WasmBinaryRow>, StoreError> {
        let conn = self.conn.lock().await;
        let mut stmt = conn
        ...
        for row in rows {
            entries.push(row.map_err(|e| StoreError::Database(format!("list_wasm_binaries row: {e}")))?);
        }
        Ok(entries)
    }
    ...
    pub(super) async fn delete_wasm_binary_impl(&self, name: &str) -> Result<(), StoreError> {
        let conn = self.conn.lock().await;
        conn.execute("DELETE FROM wasm_binaries WHERE name = ?1", rusqlite::params![name])
