...
pub fn prepare(
    ...
    // Fold the write-ahead log into the database file first. The backup
    // copies `.db` as a file, so without this every row written since the
    // last automatic checkpoint would be left behind in the `-wal` and the
    // traveller would carry a backup that silently predates their last
    // work.
    store.checkpoint().map_err(OperationError::Store)?;
...
mod tests {
    ...
    /// A row written through the live handle, still in the write-ahead
    /// log at departure, must be in the backup. `prepare` checkpoints
    /// first; without that the backup carries only what an automatic
    /// checkpoint happened to have folded in, which is a backup that
    /// looks complete and silently predates the traveller's last work.
    #[tokio::test]
    async fn test_prepare_backs_up_rows_still_in_the_write_ahead_log() {
        const LATE_MARKER: &str = "WRITTEN-JUST-BEFORE-LEAVING";

        let fx = Fixture::new();
        let store = fx.populate().await;

        // Written through the handle `prepare` is given, and never
        // closed before the backup: this lives in the `-wal` file.
        let config = SafetyConfigRow {
            window_title: LATE_MARKER.to_owned(),
            ..Default::default()
        };
        ...
        let restored = SqliteBackend::open_encrypted(&fx.db_path, TEST_DB_KEY_HEX).unwrap();
        let safety = restored.get_safety_config().await.unwrap();
        assert_eq!(
            safety.map(|c| c.window_title).unwrap_or_default(),
            LATE_MARKER,
            "the backup lost a row that was still in the write-ahead log"
        );
    }
