...
pub enum DistressSignal {
    ...
    Degraded {
        agent_id: AgentId,
        remaining_capabilities: Vec<CapabilityDecl>,
    },
}
...
mod tests {
    ...
    fn test_distress_signals() {
        ...
        let low = DistressSignal::HealthLow {
            agent_id: agent,
            health_pct: 0.2,
        };
    ...
    fn test_recovery_actions() {
        ...
        let revive = RecoveryAction::PeerRevive {
            healer,
            target,
    ...
    fn test_failure_modes() {
        let instant = FailureMode::InstantDeath;
        assert!(matches!(instant, FailureMode::InstantDeath));
