...
| Phase | Scope | State |
|---|---|---|
| 1a | Framework + connectors. Daemon, CLI, rule engine, crypto vault, WASM sandbox, 8 baseline connectors, MCP endpoint. | Present. `springtaled` serves MCP over the whole connector registry at `/mcp` (Streamable HTTP, Origin check + bearer auth). |
