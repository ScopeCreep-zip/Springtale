...
├──────────────────────────────────────────────────────────┤
│                   Integration Crates                     │
│   cooperation (42 pub modules — formations, momentum,       │
...
| Phase | Scope | State |
|---|---|---|
| 1a | Framework + connectors. Daemon, CLI, rule engine, crypto vault, WASM sandbox, 8 baseline connectors. | Present, except the MCP bridge: `springtale-mcp` exposes one connector over stdio but nothing calls it — the daemon has no MCP route and the CLI has no `mcp` subcommand. |
| 1b | Bot foundations. `springtale-bot` with command router, cooperation framework, `connector-telegram`. | Present. Cooperation framework fully extracted into `springtale-cooperation` (42 pub modules, zero internal deps) and wired through a 25-step formation tick. See [`docs/arch/AUDIT-NOTES.md §3`](docs/arch/AUDIT-NOTES.md). |
...
| 2b | Desktop + safety. Tauri 2 shell, SolidJS dashboard, canvas visualisation. Duress vault, panic wipe, travel mode, disguise tray icon (G5f), OS-wide quick-hide shortcut (G5g), destructive-action approval gate (G5b). | Shell, dashboard, canvas, duress, panic wipe, travel mode, disguise tray icon, quick-hide, approval gate, content protection, the guided visual rule builder (`RuleBuilderOverlay`), i18n (8 locales, RTL-aware) and accessibility (skip link, `aria-live` regions, screen-reader canvas navigation, high-contrast and reduced-motion styles) are all present. Not present: Tauri mobile, and user-controlled font scaling. |
