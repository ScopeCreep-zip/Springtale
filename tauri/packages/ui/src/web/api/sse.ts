...
/**
 * Close every open stream and forget it.
 *
 * The desktop calls this when the vault locks: its sidecar daemon is
 * killed at the same moment and the next unlock binds a *different*
 * loopback port, so the reconnect loops here would otherwise back off
 * against a dead port forever. Subscribers that outlive the lock get a
 * fresh connection the next time they subscribe.
 */
